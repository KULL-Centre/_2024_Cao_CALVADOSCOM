...
def calcProfiles_rc(cwd, dataset, record, cycle, ax):
    ...
    for i, m in enumerate(proteins):
        if os.path.isfile(f"{cwd}/{dataset}/{record}/{record}.npy"):
            ...
            fig.add_axes(cax)
            cb = fig.colorbar(im, cax=cax, orientation="horizontal", label=record)
            cb.ax.tick_params()
...
model = "slabCALVADOSCOMcutoff2.2_0.08_2_CA_cutoff2.0_1"
# slabC2_COM_cutoff2.0_1
CALVADOS_COM = "$\\rm{CALVADOS_{COM}}$"
fig.suptitle(CALVADOS_COM)
if IDPs:
...
models_dic = {
        model: {model: list(df_csat.index)},
    }
cycle = 0
i = 0
...
plt.tight_layout()
plt.show()
fig.savefig(f'{cwd}/paper_multidomainCALVADOS/figS5.pdf', bbox_inches='tight')
